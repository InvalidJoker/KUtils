...
class GUIType<in T : ForInventory>(
    ...
    fun createBukkitInv(holder: InventoryHolder? = null, title: String = ""): Inventory {
        return when {
            bukkitType != null -> Bukkit.createInventory(holder, bukkitType, text(title))
            else -> Bukkit.createInventory(holder, dimensions.slotAmount, text(title))
        }
    }
}
